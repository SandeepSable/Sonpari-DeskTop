		...
		savebtn.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				...
				try
				{
					...
					{
						logger.debug("Please fill up textfield");
					}
				} catch (Exception ex)
				{
					ex.printStackTrace();
...
class TableFromMySqlDatabase extends JFrame
{
	public TableFromMySqlDatabase()
	{
		...
		String sql = "SELECT * FROM SP_PRODUCT_TABLE";
		...
		table.getColumn("Action1")
		        .setCellEditor(new ButtonEditor(new JCheckBox(), table));
		table.getColumn("Action2").setCellRenderer(new ButtonRenderer());
		table.getColumn("Action2")
		        .setCellEditor(new ButtonEditor(new JCheckBox(), table));
	 ...
	 * new
	 * TableFromMySqlDatabase(); frame.setDefaultCloseOperation(EXIT_ON_CLOSE);
	...
	public Object getCellEditorValue()
	{
		if (isPushed)
		{
			...
			try
			{
				...
				{
					stmt.executeUpdate(
					        "update emp set name='" + name + "', city='" + city
					                + "' where id='" + index + "'");
					
					JOptionPane.showMessageDialog(button,
					        label + ":Update recoed success");
					
				} else if (label.equalsIgnoreCase("Delete"))
				{
					stmt.executeUpdate(
					        "delete from emp where id='" + index + "'");
					
					JOptionPane.showMessageDialog(button,
					        label + ":Delete record success");
